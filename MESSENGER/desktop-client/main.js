...
// сто рабочих мест.
//
// ДВЕ ПОДСЕТИ, ОДИН АДРЕС. Домена два — rosstat.local (10.148.12.0/22) и in.local
// (192.168.254.0/23), — но сервер один и имя у него одно: p48-srv-adm01.rosstat.local. В in.local
// заведена запись на этот же адрес и маршрут через шлюз, который его знает, поэтому клиенты обеих
// подсетей ходят по одному имени, а сертификат и удостоверяющий центр у нас одни. Корень УЦ
// раздаётся групповыми политиками в оба домена и берётся из хранилища Windows, так что serverUrl
// в этой политике нужен не для «своего имени домена», а как обычный рычаг: сменить адрес сервера
// на всех машинах разом, не пересобирая .exe.
//
