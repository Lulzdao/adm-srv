...
// Только ради envSource: он говорит, пришла переменная из .env или из окружения
// системы. Config тянет dotenv, но server.js подключает его раньше нас, так что
// повторной загрузки здесь не происходит.
const { envSource } = require("../config/config");
...
//
// Значение по умолчанию годится только когда обе службы запущены из одного
// каталога репозитория (как при разработке). На боевом сервере «Искра» обычно
// стоит отдельно — например, C:\ISKRA\iskra-server, — и тогда путь ОБЯЗАТЕЛЬНО
// задаётся переменной SHARED_CERT_DIR. Иначе платформа не найдёт хранилище,
// поднимется по HTTP, и сертификат придётся прописывать файлом через TLS_PFX —
// а это отключает и общий сертификат, и загрузку из панели.
...
function createAppServer(app, env = process.env) {
  ...
  if (!resolved) {
    // Называем каталог, в который смотрели: чаще всего причина именно в том,
    // что «Искра» стоит не там, где ожидает путь по умолчанию.
    const dir = env.SHARED_CERT_DIR || SHARED_CERT_DIR;
    console.warn(
      "[внимание] Сертификат не задан — платформа работает по HTTP, пароли и переписка идут " +
        "открытым текстом. Это допустимо только в изолированной сети.\n" +
        `           Хранилище искали здесь: ${dir}\n` +
        (env.SHARED_CERT_DIR
          ? "           Каталога нет или в нём нет server.pfx — проверьте SHARED_CERT_DIR."
          : "           Это путь по умолчанию. Если «Искра» стоит в другом каталоге, укажите " +
            "SHARED_CERT_DIR=<папка Искры>\\certs в .env.")
    );
    ...
    console.warn(
      `[внимание] Сертификат задан переменной ${name}, а не общим хранилищем. Тогда он не общий ` +
        ...
        (from === ".env"
          ...
          // Порядок подсказок — по частоте, а не по «правильности»: чаще всего
          // это «set» в том же окне cmd, из которого запускают уже не первый раз.
          // Такой переменной нет ни в реестре, ни в настройках службы, и искать
          // её там можно долго.
          : `           Где искать, по порядку:\n` +
            `             1) в ЭТОМ окне cmd: echo %${name}% — если задавали «set ${name}=…» здесь же,\n` +
            `                переменная живёт до закрытия окна и в реестре её нет. Закройте окно и\n` +
            `                откройте новое — или снимите на месте: set ${name}=\n` +
            `             2) в реестре: reg query HKCU\\Environment /v ${name}\n` +
            ...
            `           Отменить, не разбираясь где: строка «${name}=» в .env.`)
    );
  }
