  ...
  if (resolved.source !== "shared-store") {
    ...
    console.warn(
      `[внимание] Сертификат задан переменной ${name}, а не общим хранилищем. Тогда он не общий ` +
        ...
        `           Переменная пришла: ${from === ".env" ? "из файла .env" : "ИЗ ОКРУЖЕНИЯ СИСТЕМЫ, а не из .env"}\n` +
        (from === ".env"
          ? `           Уберите ${name} из .env и укажите SHARED_CERT_DIR — тогда сертификат станет общим.`
          : `           Найти и убрать: reg query HKCU\\Environment /v ${name}\n` +
            `                            reg delete HKCU\\Environment /v ${name} /f\n` +
            "           Проверьте также скрипт запуска и настройки службы (в NSSM — вкладка Environment).\n" +
            `           Быстрый способ отменить, не трогая систему: строка «${name}=» в .env.`)
    );
  }
