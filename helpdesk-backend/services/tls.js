...
function createAppServer(app, env = process.env) {
  ...
  if (resolved.source !== "shared-store") {
    ...
    console.warn(
      "[внимание] Сертификат задан переменными окружения, а не общим хранилищем. Тогда он не " +
        "общий с «Искрой» и заменить его из панели нельзя.\n" +
        "           Чтобы вернуть общий: уберите TLS_PFX/TLS_CERT и укажите SHARED_CERT_DIR.\n" +
        // Переменную легко забыть: она могла остаться в окружении системы после
        // давнего теста, и .env её НЕ перебьёт — dotenv не переопределяет то,
        // что уже есть в процессе.
        "           Если в .env этого нет, переменная задана в системе: проверьте `echo %TLS_PFX%` " +
        "и `reg query HKCU\\Environment /v TLS_PFX`."
    );
  }
