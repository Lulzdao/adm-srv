...
const rest = [
  ...
  },
  {
    ...
    hint: "Уходит автору заявки. Закрытие — отдельная категория ниже.",
    source: "helpdesk",
    severity: "info",
  ...
  },
  {
    ...
    // больше нет: по нему в базе связаны уже отправленные события и правленые
    // шаблоны. Переименование ключа стоило бы миграции ради одного слова.
    kind: "ticket_resolved",
    label: "Заявка закрыта",
    hint: "Отдельно от смены статуса: письмо «заявка закрыта» читается совсем иначе.",
    source: "helpdesk",
    severity: "info",
    recipients: RECIPIENTS.AUTHOR,
    trigger: "event",
    vars: [...TICKET_VARS, "статус", "исполнитель"],
    defaultSubject: "[{{номер}}] Заявка закрыта",
    defaultBody:
